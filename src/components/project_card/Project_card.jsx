...
export default function Project_card({
    ...
    return (
        <article className="md:hidden m-4 rounded-xl overflow-hidden shadow-md bg-(--footer-background) border border-(--border-color) h-[70vh]">
            {/* Image header */}
            <div className="w-full h-44 relative bg-[rgba(0,0,0,0.04)]">
                <img
                    src={image}
                    alt={title}
                    loading="lazy"
                ...
                />
                {/* overlay title */}
                <div className="absolute left-4 bottom-3 bg-[rgba(0,0,0,0.35)] backdrop-blur-lg px-3 py-1 rounded-full">
                    <h4 className="text-sm font-semibold text-(--button-text-color)">{title}</h4>
                </div>
            </div>

            {/* content */}
            <div className="p-4 flex flex-col h-[calc(100%-11rem)]"> {/* keeps a predictable height so footer stays visible */}
                <div className="flex items-center gap-2 mb-3">
                    {tech.slice(0, 4).map((t, i) => (
                        <span key={i} className="text-xs px-2 py-1 rounded-full bg-[rgba(255,255,255,0.02)] border border-(--border-color)">{t}</span>
                    ))}
                </div>
