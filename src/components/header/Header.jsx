...
function Header() {
    ...
    // Close menu if user resizes to desktop (prevents stuck-open menus)
    useEffect(() => {
    ...
    return (
        /* relative wrapper so the absolute dropdown is anchored correctly */
        <div ref={containerRef} className="relative">
            <div
                id="site-header"
                className="sticky top-0 z-40 flex justify-between items-center shadow-md bg-(--bg-color)"
            >
                {/* name */}
                <div className="py-3 md:py-4 pl-3 md:pl-15 text-2xl heading-text">
                    {name} {lastName}
                </div>

                {/* hamburger (mobile) - fixed clickable box so it doesn't resize layout */}
                <button
                ...
                >
                    {
                        menuOpen ? (
                            <svg xmlns="http://www.w3.org/2000/svg" className="fill-(--text-color) hover:fill-white transition duration-300" height="34" viewBox="0 -960 960 960" width="34"><path d="m256-200-56-56 224-224-224-224 56-56 224 224 224-224 56 56-224 224 224 224-56 56-224-224-224 224Z" /></svg>
                        ) : (
                            <svg
                                ...
                                width="34"
                                className="fill-(--text-color) hover:fill-white transition duration-300"
                            >
